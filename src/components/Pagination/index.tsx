...
function genereatePagesArray(from: number, to: number) {
...
export function Pagination({ totalCountOfRegisters, registerPerPage = 10, currentPage = 1, onPageChange }: PaginationProps) {
    const lastPage = Math.floor(totalCountOfRegisters / registerPerPage); // Math.floor Arredonda a divisão pra cima
    const previousPage = currentPage > 1
        ? genereatePagesArray(currentPage - 1 - siblingsCount, currentPage - 1)
        : [];

    const nextPages = currentPage < lastPage
        ? genereatePagesArray(currentPage, Math.min(currentPage + siblingsCount, lastPage))
        : [];
